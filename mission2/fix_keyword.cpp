...
enum TYPE_OF_DAY { weekDays = 0, weekEnd, maxDayType, inValidType = 0xFF };
...
class UZManager {
 public:
  ...
  void Initialize() { UZ = initializeVal - 1; }
  void update() { UZ++; }
  int getUZ() { return UZ; }
...
class SimilarityCalculator {
 public:
  bool isSimilar(const std::string& keyWord1, const std::string& keyWord2,
                 int (*algo)(const std::string&, const std::string&)) {
...
class IKeywordManager {
 public:
  ...
  virtual bool getPerfectKeyword(int index, const std::string& keyWord,
                                 keyWordManageData& target) = 0;
 ...
 public:
  ...
  void registerData(int index, const std::string& keyWord) override {
    ...
    if (weekDayBest[index].back().point <
        UZManager::GetInstance().getUZ()) {  // 가장 작은 keyword의 점수가 UZ
                                             // 값보다 작으면 pop하고 push
  ...
  bool getPerfectKeyword(int index, const std::string& keyWord,
                         keyWordManageData& target) override {
...
class Processor {
 public:
  ...
  std::string getRecommededKeyword(std::string keyWord, std::string dayString) {
    ...
    // 재정렬 작업
    if (UZManager::GetInstance().getUZ() >= ARRAGNE_THRESHOLD ||
        max1 >= ARRAGNE_THRESHOLD || max2 >= ARRAGNE_THRESHOLD) {
      UZManager::GetInstance().Initialize();
      dataBestManager->initializePoint();
      typeBestManager->initializePoint();
    }
  ...
  int getDayType(int day) {
    if (day < monDay || day >= maxWeekDay) return inValidType;
    if (day >= monDay && day <= friDay) return weekDays;

    return weekEnd;
  }
  int getDayOfWeek(const std::string& dayString) {
    const std::string dayOfWeek[maxWeekDay] = {
        "monday", "tuesday",  "wednesday", "thursday",
        "friday", "saturday", "sunday"};

    for (int day = monDay; day < maxWeekDay; day++) {
      if (dayOfWeek[day] == dayString) return day;
    }
